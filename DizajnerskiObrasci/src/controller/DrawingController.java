...
public class DrawingController {
	...
	public void btnAddLineClicked(MouseEvent click) {
		if(initialPointOfLine == null) initialPointOfLine = new Point(click.getX(), click.getY(), edgeColor);
			...
			Command cmd=new CmdAdd(line, model);
			executeCommand(cmd);
			log.addElement(cmd.toLogText());
			initialPointOfLine = null;
		}
	...
	public void btnAddRectangleClicked(MouseEvent click) {
		...
		if (dlgRectangle.isConfirmed()) {
			...
			Command cmd=new CmdAdd(rectangle, model);
			executeCommand(cmd);
			log.addElement(cmd.toLogText());
		}
	}
	...
	public void btnAddCircleClicked(MouseEvent click) {
		...
		if (dlgCircle.isConfirmed()) {
			...
			Command cmd=new CmdAdd(circle, model); //klijent kreira objekat konkretne komande i specificira mu receiver-a
			executeCommand(cmd);
			log.addElement(cmd.toLogText());
		}
	}
	...
	public void btnAddHexagonClicked(MouseEvent click) {
		...
		if (dlgHexagon.isConfirmed()) {
			...
			AdapterHexagon hexagonAdapter = new AdapterHexagon(hexagon);
			Command cmd=new CmdAdd(hexagonAdapter, model);
			executeCommand(cmd);
			log.addElement(cmd.toLogText());
		}
	}
	...
	public void updateShapeClicked() {
		...
		else if (shape instanceof AdapterHexagon) btnUpdateHexagonClicked((AdapterHexagon) shape);
	}
	...
	public void btnUpdateHexagonClicked(AdapterHexagon oldHexagon) {
		DlgHexagon dlgHexagon = new DlgHexagon();
		...
		if (dlgHexagon.isConfirmed()) {
			...
			AdapterHexagon newHexagon = new AdapterHexagon(hex);
	...
	public void btnSelectShapeClicked(MouseEvent click) {
		...
		while(iterator.hasNext()) {
			...
			if(shapeForSelection.contains(click.getX(), click.getY())) tempListOfShapes.add(model.getIndexOf(shapeForSelection));
		}
		...
		if (!tempListOfShapes.isEmpty()) {
			...
			if (!shape.isSelected()) {
				...
				Command cmd=new CmdSelect(shape, true);
				executeCommand(cmd);
				log.addElement(cmd.toLogText());
			}
			else {
				--counterOfSelectedShapes;
				Command cmd=new CmdSelect(shape, false);
				executeCommand(cmd);
				log.addElement(cmd.toLogText());
			}
			handleSelectButtons();
	...
	public void handleSelectButtons() { //za observer - osluskivanje
		if (counterOfSelectedShapes == 0) propertyChangeSupport.firePropertyChange("unselected", false, true);
			...
			propertyChangeSupport.firePropertyChange("update turn on", false, true); //ako je broj selektovanih 1 mogu da modifikujem - yahtev
	...
	public void bringToBack() {
		...
		public void undo() {
			...
			if (commands.peek() instanceof CmdSelect) handleSelect((log.get(log.size() - 1)).split("->")[0], "undo");
		...
		public void redo() {	
			...
			if (commands.peek() instanceof CmdSelect) handleSelect((log.get(log.size() - 1)).split("->")[0], "redo");
